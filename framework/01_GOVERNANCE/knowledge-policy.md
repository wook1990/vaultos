...
연결은 **프로젝트 쪽에서 시작한다.** 프로젝트를 구성하거나 판단이 필요할 때 사람 또는 에이전트가 기존 지식을 찾아 읽고 인용하는 것이지, 지식을 생성하는 시점에 먼저 "이게 어떤 프로젝트에 쓰일까"를 찾아 끼워 넣는 것이 아니다.

그래서 Curation/Caution 작업 중 Project Hooks는 **기본적으로 비워둔다.** 아래 조건 중 하나를 만족할 때만 채운다.

- 사용자가 `linked_project`를 명시적으로 입력했다
- `analysis_intent`가 `connect_to_project`다
- source 자체가 이미 진행 중인 특정 프로젝트를 직접 언급하거나 그 프로젝트의 산출물이다

이 조건이 없으면 활성 프로젝트 목록을 훑어보며 그럴듯한 연결을 찾아 붙이지 않는다. 지식은 나중에 프로젝트가 필요할 때 검색해서 찾아지는 대상으로 남겨둔다.
...
- 프로젝트 연결 후보는 위 "지식과 프로젝트의 연결 방향"의 조건을 만족할 때만 표시한다. 조건이 없으면 이 항목은 빈 채로 둔다
- 다음 결정을 명확히 남긴다
